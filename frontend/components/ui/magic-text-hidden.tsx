...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
  ...
  const renderCanvas = useCallback(() => {
    ...
    // Generate placeholder string of roughly equal width to original text
    const measuredText = measureText(text, fontSize, fontWeight, fontFamily);
    const charWidthApprox = fontSize * 0.6;
    const placeholderLength = Math.max(
      3,
      Math.round(measuredText.width / charWidthApprox)
    );
    const placeholderText = "█".repeat(placeholderLength);
